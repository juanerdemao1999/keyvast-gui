...
pub struct KvApp {
    ...
    /// DA34: latched message once a frame has panicked. While set, `update`
    /// stops driving the live pipeline and renders a static recovery screen
    /// rather than re-entering the (broken) render path.
...
impl eframe::App for KvApp {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        // DA34: once a frame has panicked, stop driving the (broken) render
        // path and show a static recovery screen; re-entering it would only
        // risk panicking again and spinning the event loop.
        if self.fatal_panic.is_some() {
            self.render_panic_screen(ctx);
            return;
        }
        // Isolate the frame: a panic in any panel is caught here so the active
        // recording can be finalized and the process kept alive long enough to
        // surface an error screen instead of vanishing mid-experiment.
